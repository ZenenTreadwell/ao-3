    ...
    .clearboth(v-for='(t, i) of priorities'  :key='t')
      .row.priority
          .priorityContainer
              ...
              .dot(@click='completed(t)' v-if='hasCompleted(t)')
              .dot(v-else  @click='prioritized(t)')
      .row.subpriority(v-if='!$store.getters.contextResource'  v-for='(st, j) of getSubPriorities(t)'   :key='st')
...
export default {
  ...
  computed:{
      priorities(){
          return this.$store.getters.contextCard.priorities
      },
  },
  components:{
      Hyperpriority,
